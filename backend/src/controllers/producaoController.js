...
exports.list = asyncHandler(async (req, res) => {
  ...
  if (req.query.q) {
    sql += ' WHERE p.prato LIKE ? OR fl.nome LIKE ?';
    const q = `%${req.query.q}%`;
    params.push(q, q);
  }
...
exports.remove = asyncHandler(async (req, res) => {
  const [result] = await pool.query('DELETE FROM producao WHERE id = ?', [req.params.id]);
  if (!result.affectedRows) return res.status(404).json({ error: true, message: 'Produção não encontrada' });
